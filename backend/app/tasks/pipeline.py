...
from .recon import run_recon_task
...
def run_full_scan_task(self: Task, target: str) -> dict[str, Any]:
    """Run passive recon for a domain, then actively scan every resolved IP.

    The pipeline first discovers subdomains with Subfinder and persists them.
    It then reads the unique IP addresses attached to those subdomains from the
    database and dispatches an active scan (Masscan + Nmap) for each IP. The
    scans run as a Celery ``group`` so every worker picks one IP and finishes
    in parallel instead of scanning sequentially.
    ...
    try:
        ...
        recon_result: dict[str, Any] = run_recon_task.run(domain)
        ...
        if not ips:
            ...
            return {
                "status": "success",
                "domain": domain,
                "subdomains_found": recon_result.get("subdomains", 0),
                "ips_to_scan": 0,
